...
<cfsilent>
	<cfscript>
		muraVersion = application.configBean.getVersion();
		updated = false;
		if( structKeyExists(form,"action") && form.action eq "updateSettings" )
		{
			updated = true;
			request.pluginConfig.setCustomSetting( "shTheme",form.shTheme );
		}
		// shLanguages = request.pluginConfig.getCustomSetting( "shLanguages" );
		theme = request.pluginConfig.getCustomSetting( "shTheme" );
	</cfscript>	
</cfsilent>
